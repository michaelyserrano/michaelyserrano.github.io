...
export const Projects: ProjectInterface[] = [
  ...
  },
  {
    ...
    techStack: ["Python", "FastAPI", "Typescript", "React", "Next.js", "PostgreSQL", "Redis", "Docker", "Celery", "Nginx", "MinIO", "Sentry", "GitHub Actions", "pgvector"],
    ...
    descriptionDetails: {
      ...
      bullets: [
        "[ Infrastructure ] 8 Docker services (FastAPI, Next.js, Celery, Redis, PostgreSQL, MinIO, Nginx, Flower) with multi-stage builds, health checks, and GitHub Actions CI/CD deploying to a self-hosted CSAIL runner.",
        "[ Backend ] Dual PostgreSQL databases (SQLAlchemy + Prisma) with pgvector for embedding storage, MinIO S3-compatible object storage with presigned URLs, and SHA-256 file deduplication. Redis for task queues, event streaming, and embedding cache with 7-day TTL.",
        "[ Load Balancing ] Nginx reverse proxy routing to frontend, backend, and Flower with rate limiting (30 req/s API), horizontal scaling via Docker Compose, gzip compression, and SSE-aware proxy buffering.",
        "[ Data Pipeline ] Three-stage LLM pipeline: ingestor (PDF/OCR text extraction), extractor (map-reduce knowledge graph), and generator (agentic quiz generation with curator planning + critique loop). Dispatched as Celery tasks with progress tracking.",
        "[ Real-time ] Quiz questions stream live via Celery workers publishing to Redis Streams, FastAPI SSE endpoints, and a Next.js proxy route — rendered in the browser as they're generated.",
        "[ Auth & Monitoring ] NextAuth v5 with CSAIL OIDC and MIT Touchstone, three-tier RBAC with server-side route guards. Sentry across FastAPI, Celery, and Next.js (client/server/edge) with performance tracing, session replay, and an ad-blocker bypass tunnel.",
        "[ Frontend ] Next.js 14 with React Flow and Dagre for the interactive concept graph editor. Vector similarity search (cosine) with sentence-boundary-aware chunking for RAG grounding.",
      ],
    },
  },
];
