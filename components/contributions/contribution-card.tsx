...
export default function ContributionCard({
  ...
  const cardContent = (contribution: contributionsInterface) => (
      ...
      {contributions.map((contribution, id) =>
        contribution.link ? (
          <Link
            ...
            target={contribution.link.startsWith("http") ? "_blank" : undefined}
            rel={contribution.link.startsWith("http") ? "noopener noreferrer" : undefined}
            key={id}
            className="w-full min-w-0 h-full"
          >
            {cardContent(contribution)}
          </Link>
        ) : (
          <div key={id} className="w-full min-w-0 h-full">
            {cardContent(contribution)}
          </div>
        )
      )}
